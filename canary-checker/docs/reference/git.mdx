...
| **Connection** |                                                              |                                                   |          |
| `connection`   | Path of an existing connection e.g. `connection://github/org`. Mutually exclusive with `credentials`  <br /> <Commercial /> | <CommonLink to="connections" >*Connection*</CommonLink>          |          |
| `githubToken`  | Github access token. <SkipOSS>Mutually exclusive with `connection`</SkipOSS>      | <CommonLink to="authentication">*EnvVar*</CommonLink> | Yes      |
