  ...
  rows={[
    {
      ...
      description: `CEL expression that determines whether all the resources are in their desired state before running checks on them. 
        ...
        The default behavior is to wait until all the resources are healthy \`dyn(resources).all(r, k8s.isHealthy(r))\`.`,
      scheme: 'CEL'
    },
    {
      field: 'disable',
...
The static and non static resources can be templated. This is helpful to create resources with random names. Example:
you can setup a resource to create a pod with random name on each check run. This way you don't have to wait for the pod to be deleted on every check.
