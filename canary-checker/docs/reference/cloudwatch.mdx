...
| **Connection** |  |  | |
| `connection` | <SkipOSS>Path of existing connection e.g. `connection://aws/instance` Mutually exclusive with `accessKey` and `secretKey` </SkipOSS> | [Connection](../reference/connections) | |
