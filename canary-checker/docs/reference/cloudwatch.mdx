...
| **Connection** |  |  | |
| `connection` | <SkipOSS>Path of existing connection e.g. `connection://aws/instance` Mutually exclusive with `accessKey` and `secretKey` </SkipOSS> | [Connection](../concepts/connections) | |
| `accessKey` | <SkipOSS>Mutually exclusive with `connection`</SkipOSS>   | <CommonLink to="authentication" anchor="envvar">*EnvVar*</CommonLink> |  |
| `secretKey` | <SkipOSS>Mutually exclusive with `connection`</SkipOSS>   | <CommonLink to="authentication" anchor="envvar">*EnvVar*</CommonLink> |  |
