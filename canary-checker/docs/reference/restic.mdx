...
| `connection`      | Path of an existing connection to get the encryption key e.g., `connection://restic/key` <br />Mutually exclusive with `password` <br /><Commercial /> | [Connection](../reference/connections)             |          |
| `password`        | <SkipOSS>Mutually exclusive with `connection`</SkipOSS>                           | <CommonLink to="authentication">*EnvVar*</CommonLink> | Yes      |
| **AWS Connection** |  |  |          |
| `awsConnectionName` | Path of an existing connection to connect to S3 e.g., `connection://aws/instance` <br />Mutually exclusive with `accessKey` <br /><Commercial /> | [Connection](../reference/connections)             |          |
