...
<HealthCheck name="redis" rows={[
  {field: "url", description: "Redis address in `host:port` format. Required unless `connection` supplies the address", scheme: 'string'},
  {field: "connection", description: "Connection URL, such as `connection://redis/prod`, that supplies Redis connection details", scheme: 'string'},
  ...
  {field: "addr", description: "Deprecated. Use `url` instead", scheme: 'string'},
  {
    field: 'tlsConfig',
...
`ssl=true`, or other query-string flags in `url`. TLS must be configured explicitly via `tlsConfig`.
:::
...
<Fields
  rows={[
    {
      field: 'enable',
      description: 'Explicitly enable TLS when no other TLS field is set (for example, for publicly trusted servers that need no CA).',
      scheme: 'bool'
    },
    {
      field: 'ca',
