...
```yaml title="jobs.yaml"  file=../../../modules/canary-checker/fixtures/prometheus/jobs.yaml
```

### Create a check per failing job only

```yaml title="jobs.yaml"  file=../../../modules/canary-checker/fixtures/prometheus/jobs-fail-only.yaml
```
