...
// CommonJS wrapper for ESM modules
async function main() {
  const { promises: fs } = await import('fs');
  const fsSync = (await import('fs')).default;
  const path = (await import('path')).default;
  // Import glob from the local node_modules 
  const globPath = path.join(process.cwd(), 'node_modules', 'glob', 'dist', 'esm', 'index.js');
  const globModule = await import(globPath);
  ...
  const remarkParse = remarkParseModule.default || remarkParseModule.remarkParse;
  const remarkMdxModule = await import('remark-mdx');
  const remarkMdx = remarkMdxModule.default || remarkMdxModule.remarkMdx;
  const remarkStringifyModule = await import('remark-stringify');
  console.log('remarkStringifyModule keys:', Object.keys(remarkStringifyModule));
  console.log('remarkStringifyModule.default type:', typeof remarkStringifyModule.default);
  const remarkStringify = remarkStringifyModule.default || remarkStringifyModule.remarkStringify;
  const visitModule = await import('unist-util-visit');
  const visit = visitModule.visit || visitModule.default;
