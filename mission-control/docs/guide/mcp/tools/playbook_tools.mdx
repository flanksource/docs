...
## Dynamic Playbook Tools

Each playbook generates a dedicated execution tool at session start. The tool name follows the pattern `{name}_{namespace}_{category}` (e.g. `deploy-app_prod_ops`). Parameters are derived from the playbook's spec.
...
**Note:** This is a destructive operation that will modify system state. Parameters are playbook-specific and can be found using the `get_all_playbooks` tool.
