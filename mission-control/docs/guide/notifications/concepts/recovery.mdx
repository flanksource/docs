...
3. The resource turns healthy. Mission Control waits for `onResolved.waitFor` of continuous health, so a resource that
   flaps straight back to unhealthy produces no recovery update.
4. Mission Control updates the original destination: a threaded Slack reply, a Slack reaction, or an email that
   replies to the original alert.
