...
The following example shows two notifications `check-alerts` and `homelab-config-health-alerts` being assigned to
a permission group "config-notifications". The group is then allowed `playbook:run` permission which is inherited by 
both notifications.
