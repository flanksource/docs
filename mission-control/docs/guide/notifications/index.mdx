...
In this example `{{.check.name}}` and `{{.check.health}}` are placeholders that gets replaced with the actual values of the `name` and `health` fields from the [check](/docs/reference/canary-checker/check) object associated with the event.
...
- This expression evaluates to true only when the `type` attribute of the [check](/docs/reference/canary-checker/check) object is equal to `'http'`.
...
For a more sophisticated filter, you can also use the handy [`matchQuery`](/docs/reference/scripting/cel#matchquery) cel function that supports search query.
Example:
