...
When a GitOps action is run against a config resource that has a `config.kubernetes.io/origin` annotation, the following [env vars](/guide/playbooks/reference/env-vars) are automatically available:
