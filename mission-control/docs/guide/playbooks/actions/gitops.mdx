...
<Fields oneOf={["connection", "url"]} rows={[
  {field: "connection", description: "Connection name to use the credentials for the git repo. When set, `username` and `password` are ignored", scheme: "string"},
  ...
  {field: "skipExisting", description: "Do not push to branches that already exist", scheme: "bool"},
  {field: "force", description: "Overwrite history when pushing", scheme: "bool"},
  {field: "type", description: "Git repository type", scheme: "`github`, `gitlab`, `azure_devops`, `http`, `git`"}

]}/>

:::warning
`force` overwrites the remote branch history. Any commits a collaborator pushed to that branch since the last clone are discarded. Leave `force` unset unless the branch belongs solely to this playbook.
:::
...
| `autoMerge.enabled`  | Merge the pull request automatically when supported | `bool`     |          |             |
| `autoMerge.rebase`   | Rebase instead of merging                          | `bool`     |          |             |
