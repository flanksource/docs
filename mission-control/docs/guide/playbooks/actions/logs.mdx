...
<Fields oneOf={["loki", "cloudwatch", "opensearch", "kubernetes"]} rows={[
  ...
  {field: "mapping.groupBy", description: "Fields to group logs by; each unique combination of values becomes a separate log group", scheme: "[]string"},
  {field: "mapping.dedupBy", description: "Fields used to identify duplicate log entries for deduplication within groups", scheme: "[]string"},
