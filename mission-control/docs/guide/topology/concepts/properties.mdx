...
Property values can be looked up from external sources (HTTP, SQL, Prometheus etc.) using the `lookup` field:
...
Properties' can also use `lookup` with `expr` field which returns a json object of `Property` or json list of `[]Property`
