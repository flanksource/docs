...
| `permissions`         | Enable fetching pipeline permissions                                  | [`Permissions`](#permissions)                         |          |
...
| `auditLog`            | Fetch organization-level audit log entries as config changes          | [`AuditLog`](#audit-log)                              |          |
...
`maxAge` defaults to the `azuredevops.pipeline.max_age` [system property](/docs/system-properties), which itself defaults to `7d`.
...
| `roles`     | Map role names to permission strings. See [Roles](#roles)                            | `map[string][]string` |
...
```yaml title="azure-devops-roles.yaml"
azureDevops:
  - permissions:
      enabled: true
      groups: true
