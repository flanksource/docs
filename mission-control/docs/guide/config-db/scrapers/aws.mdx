...
<Scraper connection="aws" rows={[
    ...
    {
      field: 'exclusions',
      description: 'Exclude individual scraped items by type, name and/or tags',
      scheme: '[`[]Exclusion`](#exclusions)'
    },
  ]}
/>
...
| ------ | ---------------------------------------------------------- | ------------------- |
| `type` | Config type patterns to exclude e.g. `AWS::EC2::Instance`  | `string`            |
| `name` | Name patterns to exclude                                   | `string`            |
| `tags` | Tags the item must carry, values matched as patterns       | `map[string]string` |

```yaml title="aws-exclusions.yaml"
aws:
  - exclusions:
...
| `lookbackDays`   | How many days of the cost and usage report to read on each scrape. Values of `0` or less use the 45 day default | `int` |
