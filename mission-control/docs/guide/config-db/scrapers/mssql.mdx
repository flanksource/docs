        ...
        script:
          javascript: |+
            ...
            var result = {
              ...
              users: data.logins.map(function(l) {
                return {
                  name: l.name,
              ...
              roles: data.server_roles.map(function(r) {
                return {
                  name: r.name,
              ...
              user_groups: data.role_members.map(function(m) {
                return {
                  user: m.member_principal_id,
                  group: m.role_principal_id
                };
              })
            };
...
If SQL Server Audit is configured, scrape audit events. See [Access Logs](/docs/guide/audit/access-logs) for more details on the access log schema.
