...
<Fields rows={[
  ...
  {
    field: "format",
    description: "Format of the objects in the bucket e.g. `Parquet`, `JSONEachRow`, `CSV`",
    scheme: "string",
    required: false
  },
  {
    field: "structure",
    description: "Column definitions for the objects e.g. `id UInt64, name String`",
    scheme: "string",
    required: false
  }
]} />

The `awsS3` block also accepts every field of an [AWSConnection](/docs/reference/connections/aws).
...
The `azureBlobStorage` block also accepts every field of an [AzureConnection](/docs/reference/connections/azure).
...
An external Clickhouse server can also be used via the `url` parameter
:::
...
spec:
  clickhouse:
    - url: "clickhouse://user:password@localhost:9000/ecommerce"
      query: |
        SELECT
          order_id,
          customer_id,
...
spec:
  full: true
  clickhouse:
    - url: "clickhouse://admin:password@localhost:9000/system"
        ...
        script:
          javascript: |+
            ...
            var result = {
              ...
              user_groups: (data.role_grants || []).map(function(g) {
                return {
                  user: g.user_name || g.role_name,
                  group: g.granted_role_name
                };
              })
            };
...
Use the system.query_log table to track who accessed what data. See [Access Logs](/docs/guide/audit/access-logs) for more details on the access log schema.
...
spec:
  full: true
  clickhouse:
    - url: "clickhouse://user:password@localhost:9000/config_db"
      type: Configuration
      id: $.config_id
      transform:
        full: true
...
spec:
  clickhouse:
    - url: "clickhouse://user:password@localhost:9000/system"
      type: Clickhouse::Database
      id: $.name
      name: $.name
      query: |
