...
<!-- Source: modules/config-db/api/v1/sql.go#SQL -->
...
| `full`      | Extract [changes](/docs/guide/config-db/concepts/changes#extracting-changes), [access logs](/docs/guide/audit/access-logs), users, groups from scraped configurations | `bool`                                             |          |
        ...
        script:
          javascript: |+
            ...
            var result = {
              ...
              // Groups (role groups)
              groups: db.role_groups.map(function(g) {
                return {
                  name: g.group_name,
              ...
              user_groups: db.memberships.map(function(m) {
                return {
                  user: m.user_id,
                  group: m.group_id
                };
...
| `user` | User who accessed the config |
          ...
          javascript: |+
            ...
            for (var i = 0; i < config.rows.length; i++) {
              ...
              results.push({
                ...
                config: row.settings,
                // Access permissions
                access: row.permissions.map(function(p) {
                  return {
                    user: p.user_id,
                    role: p.role_id
                  };
          ...
          javascript: |+
            ...
            for (var i = 0; i < config.rows.length; i++) {
              ...
              results.push({
                ...
                // Users with access
                users: (row.users || []).map(function(u) {
                  return {
                    name: u.display_name,
