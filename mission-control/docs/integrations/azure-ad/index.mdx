...
:::note
Mission Control stores users and groups as **users** and **groups**, not as config items. They appear in the access control views when linked through an [Application CRD](/docs/guide/audit/applications). Mission Control stores app registrations and enterprise applications as config items in the catalog.
:::
...
- **Groups -> Users**: Group membership is tracked through user-group mappings.
...
- If the assignment has a specific app role (not the default nil role), the role is recorded as an `Role`
