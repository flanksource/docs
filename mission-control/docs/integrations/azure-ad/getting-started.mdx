...
| `AuditLog.Read.All` | Application | Sign-in and audit logs | Only needed for [HTTP scraper](./audit-logs/http-scraper) sign-in log ingestion (requires P1/P2 license) |
...
### Verify External Users and Groups

Navigate to any Application's **Access Control** tab to see external users and groups. Alternatively, verify via the API:
...
```yaml title="entra-large-tenant.yaml"
apiVersion: configs.flanksource.com/v1
kind: ScrapeConfig
metadata:
...
Microsoft Graph API enforces per-tenant rate limits. Increase the schedule interval, reduce scope with resource selectors, or stagger multiple scrapers. See [Performance & Scheduling](#performance--scheduling) for details.
