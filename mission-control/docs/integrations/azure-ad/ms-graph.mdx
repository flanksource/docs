...
- **OAuth2 client credentials** for your Entra ID app registration (tenant ID, client ID, client secret). You can supply these inline via `oauth` and `env` fields (shown below) or through a reusable [Connection](/docs/integrations/azure-ad/getting-started#connection).
...
Both entries authenticate inline via OAuth2 client credentials. You can also reference a [Connection](/docs/integrations/azure-ad/getting-started#connection) instead of inlining the secrets.
...
Every identity string added to the `aliases` array on a user is stored in the `aliases` column of the users table. When Mission Control processes an [`access`](/docs/reference/config-db/config_access) record, it resolves user aliases by matching against these aliases.
