...
Add a custom connector that points at your Mission Control MCP URL. Leave the OAuth client ID empty. Claude discovers the authorization server and registers with CIMD or DCR.

For client-specific setup, see [Client Setup](/docs/guide/mcp/client-setup).
...
You do not configure a client ID. Point the client at the MCP server URL. The authenticating user needs the `mcp:use` permission.
