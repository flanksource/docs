...
Prometheus is an open-source monitoring and alerting toolkit that collects time-series metrics from your applications and infrastructure using a pull-based model. Mission Control integrates with Prometheus to run PromQL queries as health checks, and exposes its own Prometheus-compatible metrics endpoint so you can monitor Mission Control's health checks, scrapers, and notifications.
...
For the complete metrics reference, see the [Canary Checker metrics documentation](/docs/guide/canary-checker/concepts/metrics).
