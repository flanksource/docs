...
ClickHouse is a fast, open-source column-oriented database optimized for real-time analytical queries on large datasets, with native support for querying data in S3, GCS, and Azure Blob Storage. Mission Control integrates with ClickHouse to scrape configuration data from databases and cloud storage into the catalog.

**Use cases:**

- Import config items from ClickHouse tables and queries
