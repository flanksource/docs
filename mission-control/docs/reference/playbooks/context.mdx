...
The following variables can be used within the [CEL](/docs/reference/scripting/cel) expressions of `filter`, `if`, `delays` and `parameters.default`:
...
| `playbook`   | Playbook passed to the playbook               | [`Playbook`](/docs/reference/playbooks)                  |
| `run`        | Current run                                   | [`Run`](/docs/reference/playbooks/#run)                            |
| `params`     | User provided parameters to the playbook      | `map[string]any`                                   |
| `request`    | Webhook request                               | [`Webhook Request`](/docs/reference/playbooks/webhooks) |
...
When templating `actions` with [Go Templates](/docs/reference/scripting/gotemplate), the context variables are available as fields of the template's context object `.` eg `.config`, `.user.email`
