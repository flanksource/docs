...
import OnResolved from '../../reference/notifications/_on_resolved.mdx';
...
<Fields withTemplates="true" rows={[
  ...
  {
    field: "onResolved",
    description: "Send a recovery update to the destinations that received the original unhealthy notification. See [Recovery](/docs/guide/notifications/concepts/recovery).",
    scheme: "[`OnResolved`](#on-resolved)",
  },
  {
    field: "inhibitions",
...
Set `slack.reply`, `slack.reaction`, or both. A `slack` block that disables `reply` without a `reaction` fails
validation. See [Recovery](/docs/guide/notifications/concepts/recovery) for the supported destinations.
