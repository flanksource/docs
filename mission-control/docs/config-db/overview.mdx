...
* **Insights** - Aggreate security, cost, performance and reliability insights from AWS Trusted Advisor, Config Rules, Azure Monitor, Trivy etc.
...
* **Config Items** are individual reaources e.g. `Pod`, `EBS`, `IAM Role`, `postgres.conf` scraped from external sources like  [AWS](/config-db/scrapers/aws), [Kubernetes](/config-db/scrapers/kubernetes), [Azure](/config-db/scrapers/Azure), [Azure Devops](/config-db/scrapers/azure-devops.md) , [Github](/config-db/scrapers/github)
...
Native scrapers ingest config items from common sources like [AWS](/config-db/scrapers/aws), [Kubernetes](/config-db/scrapers/kubernetes), [Azure](/config-db/scrapers/azure) and automatically add metadata and relationshops
</Step>

<Step step={2} name="Custom" style="list">

Custom scrapers ingest raw data from [Files](/config-db/scrapers/file) and [SQL queries](/config-db/scrapers/sql) the results of which need to mapped to metadata and relationships manually.
