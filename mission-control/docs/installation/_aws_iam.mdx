...
import Domain from '../partials/_domain.mdx'
...
<Step step={2} name="Configure IAM Roles for Mission Control">
<Tabs>
...
2. Create `main.tf`
	```hcl title="main.tf"  file=../partials/_pod_identity.tf
	```
...
1. Associate Pod Identities
	```yaml title="mission-control-iam-cloudformation.yaml"  file=../partials/_pod_identity.yaml
	```
3. Install Mission Control
	<Helm chart={props.chart}/>
...
First we create a secret called `aws` containing the access key and secret, This secret is not s
...
3.   Create a new connection to use bundles
