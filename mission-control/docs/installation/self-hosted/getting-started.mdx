...
1. Install Mission Control
    <p/>
    :::tip
    If you are installing on EKS and plan to scrape AWS resources use the method described in [EKS](/docs/installation/self-hosted/eks) instead
    :::
...
<Cards className="border border-t border-solid border-gray-200">
  ...
  <Card size="sm" title="Use an External Database" icon="postgres" link="/docs/installation/self-hosted/database"/>

  <Card size="sm" title="Configure Single Sign-On" icon="jwt" link="/docs/installation/self-hosted/sso"/>
  <Card size="sm" title="Configure SMTP/Email" icon="email" link="/docs/installation/self-hosted/smtp"/>
