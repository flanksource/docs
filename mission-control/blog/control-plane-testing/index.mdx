...
The [`kubernetesResource`](/docs/guide/canary-checker/reference/kubernetes-resource) check creates kubernetes resources based on the provided manifests & perform checks on them, it has 5 lifecycle stages:
...
2. Add custom check
        ...
        This example uses the `$(username)` and `$(password)` syntax to access the `username` and `password` variables hardcoded in the `checks` section, but in a production setting, reference secrets using [`valueFrom`](/docs/reference/env-var)
    :::
...
- [kubernetesResource](/docs/guide/canary-checker/reference/kubernetes-resource) Canary
