...
Me and [Aditya](https://adityathebe.com) ran multiple profiles for a few hours but didn't get anything conclusive. The only surity was whatever was causing the crash was instant, and not a slow over the time memory leak.
...
It looks something like ([Link to full code](https://github.com/yashmehrotra/go-rust-diffgen/blob/main/main.go)):
```go
package main
...
and the rust code ([Link to full code](https://github.com/yashmehrotra/go-rust-diffgen/blob/main/lib/diffgen/src/lib.rs)):
...
The `lib/hello.h` header file ([Link to full code](https://github.com/yashmehrotra/go-rust-diffgen/blob/main/lib/diffgen.h)):
```c
...
We timeboxed the effort, and within a day I made a working concept with our own codebase. The benchmarks againsts our existing test suite gave promising results.
...
- [Offical Rust FFI documentation: Using `extern` Functions to Call External Code](https://doc.rust-lang.org/book/ch19-01-unsafe-rust.html#using-extern-functions-to-call-external-code)
- [Medi-Remi's sample repo: rust-plus-golang](https://github.com/mediremi/rust-plus-golang)
- [Filippo's blog: rustgo: calling Rust from Go with near-zero overhead](https://words.filippo.io/rustgo/)
- [MetalBear's blog: Hooking Go from Rust - Hitchhiker’s Guide to the Go-laxy](https://metalbear.co/blog/hooking-go-from-rust-hitchhikers-guide-to-the-go-laxy/)
