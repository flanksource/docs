...
export default function BlogAuthor({ as, author, className, count, short = false }) {
  ...
  return (
    <div
      className={clsx(
        ...
        className,
        short && styles['author-as-short'],
        styles[`author-as-${as}`],
      )}>
      {imageURL && (
        <MaybeLink href={link} className="avatar__photo-link">
          <img
            className={clsx('avatar__photo', short && "w-6 h-auto")}
            src={imageURL}
            alt={name}
          />
        </MaybeLink>
