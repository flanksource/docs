...
1. Filtering a list to include only numbers greater than 2:
```cel
[1, 2, 3, 4].filter(e, e > 2)  // Evaluates to [3, 4]
```

2. Filtering a map to include only entries with values greater than 1:
```cel
...
1. Checking if all elements of a list are greater than 0:
```cel
[1, 2, 3].all(e, e > 0)  // Evaluates to true
```

2. Checking if all values of a map are non-empty strings:
```cel
{"a": "apple", "b": "banana", "c": ""}.all(k, v, v != "")  // Evaluates to false
```

3. Using both key and value for condition in a map:
```cel
...
1. Checking if any element of a list is equal to 2:
```cel
[1, 2, 3].exists(e, e == 2)  // Evaluates to true
```

2. Checking if any value of a map is an empty string:
```cel
...
1. Computing the sum of all elements of a list:
```cel
[1, 2, 3].fold(e, acc, acc + e)  // Evaluates to 6
```

2. Concatenating all values of a map:
```cel
